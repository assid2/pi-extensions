...
const out = [
  ...
  `// Entries: ${Object.keys(limits).length} (last run: probed ${probed}, ${failed} failed)`,
  "",
...
console.log(
  `Wrote limits.generated.ts (${Object.keys(limits).length} entries; last run probed ${probed}, ${failed} failed).`,
);
