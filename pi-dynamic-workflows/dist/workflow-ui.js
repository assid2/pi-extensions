...
function rightAgentRow(a, selected, modelColStart, innerW, theme, runId) {
...
function feedRunAgents(model, runId, now) {
...
function sumAgentRates(model, runId) {
    let total = 0;
    for (const [, agents] of model.agentsByPhase(runId)) {
        for (const a of agents) {
            if (a.status === "running")
                total += agentTokensPerSecond(runId, a.id);
        }
    }
    return total;
}
function renderPhasesAgents(state, model, runId, width, theme, bodyCap, now) {
...
function renderSinglePane(state, phases, selPhaseIdx, agents, width, theme, bodyCap, inAgents, runId) {
...
export function renderNavigator(state, model, width, theme = PLAIN, viewportRows = 24, markdownTheme, now = Date.now()) {
    return model.withRenderFrame(() => renderNavigatorFrame(state, model, width, theme, viewportRows, markdownTheme, undefined, now));
}
function renderNavigatorFrame(state, model, width, theme, viewportRows, markdownTheme, renderCache, now = Date.now()) {
    const lines = [];
    let visibleSnapshot;
