...
import { preview, recomputeWorkflowSnapshot } from "./display.js";
import { isProviderUsageLimit, WorkflowError, WorkflowErrorCode } from "./errors.js";
import { agentHasNonTerminalStatus, createRunPersistence, generateRunId, INTERRUPTED_AGENT_CAUSE, settleInterruptedPersistedAgents, settleNonTerminalPersistedAgents, terminalRunInterruptCause, } from "./run-persistence.js";
    ...
    recoverStaleRuns() {
        try {
            for (const p of this.listAllRuns()) {
                ...
                const pausedWithGhostAgents = p.status === "paused" && p.agents.some((agent) => agentHasNonTerminalStatus(agent.status));
